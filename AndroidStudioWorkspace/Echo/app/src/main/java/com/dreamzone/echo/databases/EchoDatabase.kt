package com.dreamzone.echo.databases
...
class EchoDatabase : SQLiteOpenHelper {
    ...
    override fun onCreate(sqlDB: SQLiteDatabase?) {
        sqlDB?.execSQL(
            "CREATE TABLE " + Staticated.TABLE_NAME + "( " + Staticated.COLUMN_ID +
                    " INTEGER," + Staticated.COLUMN_SONG_ARTIST + " TEXT," + Staticated.COLUMN_SONG_TITLE +
                    " TEXT," + Staticated.COLUMN_SONG_PATH + " TEXT);"
        )
    }
    ...
    fun queryDBList(): ArrayList<Songs>? {
        try {
            ...
            val queryCursor = db.rawQuery(query_params, null)

            if (queryCursor.moveToFirst()) {
                do {
                    var _id = queryCursor.getInt(queryCursor.getColumnIndexOrThrow(Staticated.COLUMN_ID))
                    var _artist =
                        queryCursor.getString(queryCursor.getColumnIndexOrThrow(Staticated.COLUMN_SONG_ARTIST))
                    var _title =
                        queryCursor.getString(queryCursor.getColumnIndexOrThrow(Staticated.COLUMN_SONG_TITLE))
                    var _songPath =
                        queryCursor.getString(queryCursor.getColumnIndexOrThrow(Staticated.COLUMN_SONG_PATH))
    ...
    fun checkifIdExists(_id: Int): Boolean {
        ...
        val checkCursor = db.rawQuery(query_params, null)

        if (checkCursor.moveToFirst()) {
            do {
                storeId = checkCursor.getInt(checkCursor.getColumnIndexOrThrow(Staticated.COLUMN_ID))
